...
| `pastEnd` | `1.5` | Near a stroke's end and then beyond it along the stroke's own direction by more than this many end tolerances without having reached it, the stroke starts over: progress back to its start, the stroke in hand discarded at the lift and not recorded, a toast saying why. The hand had been poking the end to be let on after missing it a little above or below. `0` switches it off. Not in guided, whose end is reach. |
...
| *(the dashboard)* | | Two bars on the seam — 命 life (the ward) and 気 energy — with 墨, 魂 and the wave, and tabs 筆 (the sketchbook), 技 (this run's intent), 耐 (persistence) and 志 (motivation: currency gain), all bought with ink, and 灯 (the lanterns, bought with 魂). A tab does not pause the run, and a shop stands where the sketchbook stood: while one is open nothing can be traced, and a shop does not open under a pen that is down. |
...
| `field.noRepeat` | `4` | In a character realm the tracer asks from its own queue over the open rows — shaky characters first, then least recently written — and no character comes round again until this many others have. What is drawn hits whichever farang carries it; nothing on the field changes what is asked. |
| `field.biteRate` | `0.5` | How often a spawn is drawn from the characters this hand keeps getting wrong (the ledger's `shaky`) rather than the whole roster. The maintainer: "I always mess up も ... that reflection should be used against me." |
...
| `pen`, `finger` | see above | the two hands, tuned apart. `size` is a `[min, max]` glyph size for that hand, or `null` to leave the pack's own rule alone (a size a difficulty pins, like guided's, is never overridden). The packs pin the pen at 0.52 since v0.1.47: over 239 medium traces the 0.48–0.56 band had the fewest zaps and the most clean traces, and the maintainer found the random sizes an annoyance. `stage` is the sketchbook's size in the games. `ease` multiplies the path's forgiveness (1–2; the scorer caps the result and every end-of-stroke guard is still bound by the stroke's own length). `trail` widens the road ahead, and `halo` is the radius in px of a ring around the light — both so a fingertip does not hide what it is steering by. The finger profile applies in finger mode on a touch screen, and lets go while a pen is down. `endFloor` (v0.1.52) is the least distance, as a fraction of the canvas, within which a stroke's end or start counts as reached: it replaces `minEndTolerance` per hand. A pen lands within 2%; a fingertip cannot, and on ふ's hook the hand had to "trace, and poke until it allows me to get to the next stroke". `flick` (v0.1.69; finger 0.12, pen 0) is the length, as a fraction of the canvas, under which a stroke is a flick for this hand: it lands if it began near its start (1.25× the start tolerance), moved a third of its length in its direction, and ended near its end (1.25× the end tolerance), and one that went nowhere is dropped, never a restart. 点々 and ふ's ticks; by finger the voiced kana fizzled 33% against 13% before it, with a short tick wiping the whole character. The finger's `ease` went 1.5 → 1.3 and its `stage` 42 → 46dvh in v0.1.48 after the maintainer traced medium with three different fingers: at 1.5 the band the scorer forgives was 34% of the glyph wide (a quarter circle passed for a loop); at 1.3 it is 28%, against a pen's 25%. A bigger sketchbook leaves that ratio alone — the band and the glyph are both fractions of the canvas — but shrinks the fingertip and its touch offset relative to the glyph, which is what "my strokes don't register where I'm touching" is. |
