...
**The dashboard (v0.1.55), and 気 (v0.1.56).** Two bars on the seam between
field and sketchbook, where the eyes already are — 命, the ward, and 気, the
energy for casting: one bag every stroke fills whatever is on the field, and
...
not allow the player to progress the game." The handwriting is still recorded.

**As of v0.1.51 the games have two modes: guided (practice) and medium (the
game).** Easy is gone from the shells — the maintainer: "noticed how crappy
easy mode is, and it's pretty much guided mode.. so.. let's just remove it.
only score you can get is from medium" — though the engine's easy mode
remains for the workshop. **Bosses**: the last farang of every stage takes
more hits and is traced with less help, the shape faint and the start dot
...
- **A layer must not reuse an engine name.** Inside a layer's function,
  ...
  source for it. It has been a shipped bug twice. **Class names count**: the
