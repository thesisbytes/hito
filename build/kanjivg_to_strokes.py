...
# --- alignment to the rendered glyph -------------------------------------
# The engine stores teacher strokes in canvas-normalised coordinates matching
# the glyph *as the font draws it*, not as a full-canvas 0..1 box. Thai
# recordings satisfied that for free — they were captured by tracing over the
# rendered glyph. KanjiVG's do not, and land 1.44x too large without this.
#
# The engine draws at font size BASE_F x canvas height, anchored with
# textBaseline='middle' and nudged down by 0.06 font sizes. Canvas anchors
# 'middle' at the middle of the em square, which for Klee One (upm 1000,
# typo ascender 880, descender -120) sits 0.38 em above the baseline.
#
# KanjiVG's 109 box is the em square, so:
#     X = 0.5 + BASE_F * (kx - 0.5)
#     Y = 0.5 + BASE_F * (ky - GLYPH_CY)
BASE_F = 0.62          # must match the engine's BASE_F
EM_MIDDLE = 0.38       # em middle above baseline, in ems
FILL_DROP = 0.06       # the engine's fillText y nudge, in font sizes
GLYPH_CY = EM_MIDDLE + FILL_DROP
...
def convert(svg_path):
    ...
    for d in ds:
        ...
        strokes.append([
            {
                "x": round(0.5 + BASE_F * (x / VIEWBOX - 0.5), 4),
                "y": round(0.5 + BASE_F * (y / VIEWBOX - GLYPH_CY), 4),
                "p": 1.0,
...
def main():
    ...
    book = {
        ...
        "note": ("Generated by build/kanjivg_to_strokes.py. Stroke centrelines "
                 ...
                 "textbook forms but are not glyph-exact. Coordinates are "
                 f"aligned to the glyph as rendered at BASE_F={BASE_F}, not to "
                 "the full canvas."),
        "alignment": {"baseF": BASE_F, "glyphCy": round(GLYPH_CY, 4)},
    }
