...
def main():
    ...
    if pack.get("strictFollow"):
        ...
        s.sub("strict follow",
              ...
              "    //\n"
              "    // Dragging the light (guided) has no ink to judge, and the light\n"
              "    // rests on the nearest point to a fingertip that hides the end.\n"
              "    // Asking that fingertip's centre to land inside a pen's end\n"
              "    // tolerance left 54 of the maintainer's 88 guided strokes\n"
              "    // dragging on past the end, a third of the stroke on average,\n"
              "    // waiting for a close that came only when the centre crossed the\n"
              "    // point. There the test is reach: the light within R of the end\n"
